...
def update_json():
    try:
        ...
        print(f"Error loading data: {e} Pls check the file integrity of file existence.")
        exit()


def acc_no_generation():
    while True:
        acc_no = str(random.randint(0000, 9999)).zfill(4)  # To avoid the number genera tion with leading zeros
        ...
        for user in users:
            if user['account number'] == account_number:
                ...
                if amount < 0:
                    ...
                    # break
                    return
                print("Incorrect number pls enter the correct account number")
    except ValueError:
...
while running:
    print("Options")
    print("add - To open a new account")
    print("bal - To check the balance")
    print("dep - To deposite money")
    print("wit - To withdraw money")
    print("exit - Exit")
